<template>
  <div>
    <header class="header">
      <h1>所有文章</h1>
      <p>在这里可以管理你的所有文章，包括新增，编辑，导出，导入，删除等功能</p>
    </header>
    <div class="main">
      ...
      <el-table :data="novel" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="标题" prop="title" show-overflow-tooltip>
          <template slot-scope="scope">
            <a :href="'/#/view/user/novel/' + scope.row.id" class="novelTableTitle" target="_blank">{{ scope.row.title }}</a>
          </template>
        </el-table-column>
        <el-table-column
          label="创建时间"
...
export default {
  ...
  methods: {
    /* 查询 */
    queryAll() {
      var username = this.$store.getters.getUser.username
      var url = 'http://localhost:8080/novel/username/' + username
      this.$axios({
        method: 'get',
        url: url,
        params: { page: this.currentPage - 1, length: this.length },
      })
        .then((res) => {
          console.log(res);
          var r = res.data
    ...
    addSubmit() {
      this.$axios
        .post('http://localhost:8080/novel/add', {
          username: this.$store.getters.getUser.username,
          data: this.add.editor.txt.html(),
    ...
    beforeUpload(file) {
      ...
      if (t != '' && t != null) {
        return new Promise((resolve, reject) => {
          this.$nextTick(() => {
            this.imp.action = 'http://localhost:8080/novel/import/' + this.$store.getters.getUser.username + '/' + t
            resolve()
          })
        })
      } else if (t == null) {
    ...
    exportNovelByIds() {
      ...
      if (ids.length != 0) {
        ids.forEach((id) => {
          var url = 'http://localhost:8080/novel/export/' + this.exp.type + '/' + id
    ...
    modClick(id) {
      ...
      this.$axios.get('http://localhost:8080/novel/id/' + id).then((res) => {
        var r = res.data
        if (r.state) {
    ...
    modDialogOpen() {
      if (typeof this.mod.editor == 'string') {
        ...
        this.mod.editor.txt.html(this.mod.novel.data)
      }
    },
    handleModClose() {
      this.$confirm('确认关闭？')
    ...
    modSubmit() {
      this.mod.novel.data = this.mod.editor.txt.html()
      this.$axios
        .put('http://localhost:8080/novel/modify', this.mod.novel)
        .then((res) => {
          var r = res.data
          if (r.state) {
            this.$message.success(r.msg)
